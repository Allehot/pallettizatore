...
class RecursiveFiveBlockPlanner:
    ...
    def plan_layer(self, request: LayerRequest) -> LayerPlan:
        ...
        for orientation in request.allowed_orientations():
            ...
            if not candidate.placements:
                continue
            candidate = self._apply_reference_frame(candidate, request)
    ...
    def _apply_reference_frame(self, plan: LayerPlan, request: LayerRequest) -> LayerPlan:
        frame = request.reference_frame
        transformed = [
            LayerPlacement(
                box_id=placement.box_id,
                position=frame.transform(
                    placement.position,
        ...
        plan.placements = transformed
        plan.metadata = {
            **plan.metadata,
            "reference_origin": frame.origin,
            "reference_axes": frame.axes_token,
        }
        return plan
